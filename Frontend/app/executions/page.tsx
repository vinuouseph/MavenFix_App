...
import { getExecutionHistory, ExecutionHistoryDTO } from '@/lib/api';

export default function ExecutionsPage() {
  const [requests, setRequests] = useState<ExecutionHistoryDTO[]>([]);
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, paddingTop: 32, paddingBottom: 80 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 40, flexWrap: 'wrap', gap: 16 }}>
            <div>
              <h1 style={{ fontSize: 32, fontWeight: 900, marginBottom: 6, letterSpacing: '-0.02em' }}>
                Execution <span style={{ background: 'linear-gradient(135deg, #10b981, #06b6d4)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>History</span>
              </h1>
              <p style={{ color: '#64748b', fontSize: 15 }}>
                Track the history of all pipeline executions.
              </p>
            </div>
            <div>
              <button onClick={loadRequests} disabled={loading} className="btn btn-ghost btn-sm" title="Refresh executions" style={{ opacity: loading ? 0.5 : 1 }}>
          ...
          {!loading && !error && requests.length === 0 && (
            <div className="empty-state">
              <h3 style={{ fontSize: 18, fontWeight: 700 }}>No execution history yet</h3>
              <p style={{ color: '#64748b', fontSize: 14 }}>Once a pipeline execution completes, it will appear here.</p>
            </div>
          )}
                  ...
                  {requests.map(req => (
                    <tr key={req.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', transition: 'background 0.2s', ':hover': { background: 'rgba(255,255,255,0.01)' } } as any}>
                      ...
                      <td style={{ padding: '16px 24px', fontSize: 14, color: '#cbd5e1' }}>
                        {req.executed_datetime ? new Date(req.executed_datetime).toLocaleString() : 'N/A'}
                      </td>
                      <td style={{ padding: '16px 24px' }}>
                        <span style={{ 
                          ...
                          fontSize: 12, 
                          fontWeight: 600, 
                          background: `${getStatusColor(req.result)}20`, 
                          color: getStatusColor(req.result),
                          border: `1px solid ${getStatusColor(req.result)}40`
                        }}>
                          {req.result}
                        </span>
                      </td>
